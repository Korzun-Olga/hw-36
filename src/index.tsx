...
const allGenreOfMovies = movies.map((movie:IMovie) => movie.genre).join(',').split(',')
const genreWithoutDuplicate = Array.from(new Set(allGenreOfMovies))

// 2. Собрать в массив всех актеров всех фильмов (без повторения)
const allActors = movies.map((movie:IMovie) => movie.actors).join(',').split(',')
const actorsWithoutDuplicate = Array.from(new Set(allActors))

// 3. Отсортировать фильмы по рейтингу по убыванию
const getMoviesRating: Array<IMovie> = [...movies].sort((a:IMovie, b:IMovie) => b.imdbRating- a.imdbRating)

// 4. Создать новый массив, где объекты фильмов будут состоять из следующих полей:
const getNewMovieArray = (movies:Array<IMovie>) => movies.map ((movie:IMovie)=>
{
    return {
        id: movie.id,
        title: movie.title,
...
const showArrayWithYear = (array:Array<IMovie>, number:number) => array.filter(array=> array.year===number)
showArrayWithYear (movies, 1977)
